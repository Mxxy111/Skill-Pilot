...
- 自定义 AI：兼容 OpenAI Chat Completions API 和本地 Ollama，可做批量分类、仓库识别和候选仓库语义推荐
- 自动维护：只跟踪由“发现”安装且来源完整的 Skills；按默认分支检查更新，更新前复检并备份，高风险变化自动停止
...
- `SkillPilot-Setup-0.5.0-x64.exe`：安装版，有安装向导、桌面/开始菜单快捷方式和标准卸载入口
- `SkillPilot-Portable-0.5.0-x64.exe`：免安装版，单文件双击运行，不写入程序安装目录或创建卸载项
