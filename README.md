...
- 自动维护：只分类未分类或内容已变化的 Skill，稳定结果不会周期性覆盖；来源更新前会复检并备份，高风险变化自动停止
...
- `SkillPilot-Setup-0.8.0-x64.exe`：安装版，有安装向导、桌面/开始菜单快捷方式和标准卸载入口
- `SkillPilot-Portable-0.8.0-x64.exe`：免安装版，单文件双击运行，不写入程序安装目录或创建卸载项
...
AI 分类使用 10 个稳定大类：开发与工程、数据与分析、科研与学术、写作与内容、设计与多媒体、自动化与效率、安全与审计、运维与云、产品与业务、通用工具。模型返回的近义词或过细类别会被程序收敛到其中之一。自动维护只处理未分类或内容指纹发生变化的 Skill；手动选中后运行 AI 分类才会强制刷新稳定结果。
